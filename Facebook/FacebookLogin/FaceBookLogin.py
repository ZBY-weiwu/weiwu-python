...
class Facebook_Login:
    ...
    def __init__(self):
        ...
        self.session.proxies = {'http': 'http://127.0.0.1:1081', 'https': 'https://127.0.0.1:1081'}
    ...
    def login(self, email: str, password: str):
        ...
        if login_error:
            warnings.warn("Account password error")
            return "Account password error"
        if "Login approval needed" in response.text or "checkpoint" in response.url:
            warnings.warn("Check whether the Facebook account is blocked or locked")
            return "检查Facebook账号是否被封，或锁定"
        if 'c_user' not in requests.utils.dict_from_cookiejar(self.session.cookies):
...
if __name__ == '__main__':
    ...
    # print(Facebook_Login().login("16506915048","qq116108"))
    print(Facebook_Login().login("lonelychen@live.cn", "2qazXDE#@!"))
