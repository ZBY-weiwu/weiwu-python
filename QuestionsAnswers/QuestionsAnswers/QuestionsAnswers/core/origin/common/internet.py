...
class URLUtils(object):
    ...
    @classmethod
    def get_urls_from_html_by_xpath(cls, html, pattern=None, base_url=None, tags=None, attribs=None):
        """
        从html中通过xpath获取url
        ...
        for element in elements:
            for tag in tags:
                for attrib in attribs:
                    urls.extend(element.xpath('.//%s/@%s' % (tag, attrib)))
        if base_url:
            urls = [parse.urljoin(base_url, url) for url in urls]
        return urls
...
class HTMLContentXpathParser(object):
    ...
    def get_data_by_xpath(cls,
        ...
        if tags_handler is None:
            tags_handler = {}
        content_elements = HTMLElementUtils.get_elements_by_xpath(html, pattern)
    ...
    def __get_data(cls,
        ...
        if merge:
            text = '%s%s' % (
                text, merge_sep.join(text_list).strip())
            text = text.strip()
        if raw_content_enabled:
