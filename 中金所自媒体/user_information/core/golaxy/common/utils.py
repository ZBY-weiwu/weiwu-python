...
from core.origin.common import func
from core.origin.common import request
...
class GolaxyHTMLContentXpathParser(internet.HTMLContentXpathParser):
    ...
    @classmethod
    def get_content_by_xpath(cls, html, base_url, *args, **kwargs):
        """
        通过xpath获取正文
        :param html: html页面
    ...
    def start_handle_content(cls, data_element, tags_handler, handle_elements, data):
        ...
        for handle_element in handle_elements:
            if handle_element[1]:
                ...
                handle_func_name = 'handle_%s_by_pattern' % handle_element[2]
                if hasattr(cls, handle_func_name):
                    getattr(cls, handle_func_name)(handle_element[2], data_element, handle_element[4], data,
                                                   local='tail')
            handle_elements.remove(handle_element)
...
class GolaxyProxyUtils(internet.ProxyUtils):
    DOMESTIC_HTTP_PROXIES = [  #'10.20.18.100:7777',
                             '10.1.101.52:7777']
    # DOMESTIC_HTTP_PROXIES = ["10.1.101.53:7111",
    #                          "10.1.101.53:7112",
    #                          "10.1.101.53:7113   ",
    #                          "10.1.101.53:7114   ",
    #                          "10.1.101.53:7115   ",
