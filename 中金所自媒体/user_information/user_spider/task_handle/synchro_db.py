...
import sys
from currency_module.DBConnect.py_db import EsDB
from currency_module.DBConnect.py_db import PgDB
from currency_module.Tools.by_package import format_pubtime
from tools.golaxy_package import get_past_times



class synchro_weibo_task():

    past_times = str(get_past_times(3) * 1000)

    @classmethod
    def write_pg(cls):
        es = EsDB.Es_connect()
        query_json = {"query": {"bool": {"must": [{"terms": {"uid": ["1182426800", "1569160390", "1683954217", "2121667213", "1885448847", "ikimono", "2369862891", "1743775150", "3981961007", "1686990012", "1980916411", "12256223", "1410248531", "1980337107", "1296760080", "1192082717", "1885355607", "830505568", "2377371197", "1233227211", "2626066165", "2637695114", "1227422602", "1402007153", "1296904573", "1163218074", "1211281564", "1661504227", "1594643405", "2144596567", "2499262997", "6159464207", "1863057363", "1661504227", "1483330984", "5872484124", "1288764341", "1249424622", "1168504694", "1697139145", "2150892230", "1495033135", "1769173661", "1721825977", "2337530130", "2665539323", "1210071525", "1223237202", "1222713954", "1318745373", "1593165014", "5613529675", "2556409705", "7588499708", "1965900874", "1226106902", "1803814487", "1645823934", "7654387664", "5884573948", "3810246137", "1893601564", "1893601564", "1814312747", "2030597815"]}},{"range": {"gt": {"gt": cls.past_times}}}]}}}
        index_name = "all_weibo"
        es, every_num, scroll_id = EsDB.es_scroll_scroll(es, query_json,index_name)
        for i in range(0, every_num +1):
            query_scroll = es.scroll(scroll_id=scroll_id, scroll='20m')['hits']['hits']
            # results += query_scroll
            results = query_scroll
            item_list = []
            sql = """insert into sjzt_wcm_website_zjs (id,site_id, site_name,url,board_id,detail_id,update_time,create_time,create_time_stamp,detail_url,update_time2) VALUES (%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
    ...
    def write_pg(cls):
        ...
        for i in range(0, every_num +1):
            ...
            for key in results:
                ...
                item["board_id"] = source["i_bid"]
                item["url"] = source["url"]
                item["site_name"] = source["i_sn"]
                if item["site_name"]=="头条号":
                    item["site_id"] = 33798
                    item["site_name"] = "今日头条"
                ...
                elif item["site_name"] == "东方财富App" or item["site_name"] == "东方财富APP":
                    item["site_name"] = "东方财富网股吧"
                    item["site_id"] = 7010
                    detail_id=re.findall("http://mguba\.eastmoney\.com/mguba/article/\d+/(\d+)", source["url"])
                    detail_id = detail_id[0]
                    item["detail_id"] =detail_id
                    source["url"] = "http://guba.eastmoney.com/news,gssz,{detail_id}.html".format(detail_id=detail_id)
                ...
                print(item_data)
                # print(json.dumps(item, ensure_ascii=False))
                params = (item["id"],item["site_id"], item["site_name"],source["url"],item["board_id"],item["detail_id"],0,item["create_time"],item["create_time_stamp"],item["detail_url"],0)
                PgDB.Pg_write(sql, params)
                # item_list.append(params)
            # print(item_list)
            # PgDB.Pg_list_write(sql,item_list)

if __name__ == '__main__':
    synchro_task.write_pg()
